...
<template>
    <div class="flex flex-col gap-4">
        <p class="text-sm leading-relaxed text-neutral-500">
            Si nécessaire, vous pouvez vous déconnecter de toutes vos autres sessions sur d'autres navigateurs et
            appareils.
        </p>

        <!-- Sessions list -->
        <div v-if="sessions.length" class="flex flex-col gap-3">
            <div v-for="(s, i) in sessions" :key="i" class="flex items-center gap-3 rounded-lg bg-neutral-50 p-3">
                <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-white shadow-sm">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-xs text-neutral-400">
                        {{ s.ip_address }} ·
                        <span v-if="s.is_current_device" class="font-semibold text-green-600">Cet appareil</span>
        ...
        <div class="flex items-center gap-3">
            <AppButton variant="secondary" @click="confirm"> Déconnecter les autres sessions </AppButton>
        ...
        <DialogModal :show="confirmingLogout" @close="closeModal">
            ...
            <template #footer>
                <div class="flex justify-end gap-2">
                    ...
                    <AppButton :loading="form.processing" @click="submit"> Se déconnecter </AppButton>
                </div>
            </template>
        </DialogModal>
    </div>
