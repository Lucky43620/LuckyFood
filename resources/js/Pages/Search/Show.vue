...
const { number: fmt } = useFormatting()
...
<template>
    <MainLayout :title="food?.food_name ?? 'Aliment'">
        <div class="max-w-3xl px-5 py-6 md:px-8">
            ...
            <div
                v-if="searchError"
                class="mb-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800"
            >
                {{ searchError.message }}
            </div>
            ...
            <div
                v-else-if="!food"
                class="rounded-2xl bg-white p-14 text-center"
                style="
                    box-shadow:
            ...
            <div v-else class="space-y-6">
                ...
                >
                    ...
                    <div class="relative overflow-hidden" style="height: 180px; border-radius: 20px 20px 0 0">
                        ...
                        <div
                            ...
                            style="
                                background: linear-gradient(135deg, oklch(62% 0.18 145) 0%, oklch(54% 0.17 145) 100%);
                            "
                        >
                            <span
                                class="absolute select-none font-display font-black text-white"
                    ...
                    <div class="p-5 md:p-6">
                        ...
                        <h1 class="font-display text-[26px] leading-tight text-neutral-900 md:text-[32px]">
                            {{ food.food_name }}
                        </h1>
                        <p
                            v-if="food.original_food_name && food.original_food_name !== food.food_name"
                        ...
                        <div class="mt-5 grid grid-cols-4 gap-3 border-t border-neutral-100 pt-5">
                            <div>
                                <p class="font-mono text-4xl font-black leading-none text-neutral-900">
                                    {{ fmt(calories, 0) }}
                                </p>
                                <p class="mt-1.5 text-[11px] font-bold uppercase tracking-wide text-neutral-400">
                                    kcal
                                </p>
                            </div>
                            <div>
                                <p
                                    class="font-mono text-4xl font-black leading-none"
                                    style="color: oklch(60% 0.16 240)"
                                >
                                    {{ fmt(protein, 1) }}
                                </p>
                                <p
                                    class="mt-1.5 text-[11px] font-bold uppercase tracking-wide"
                                    style="color: oklch(70% 0.14 240)"
                                >
                                    Prot. g
                                </p>
                            </div>
                            <div>
                                <p class="font-mono text-4xl font-black leading-none text-amber-500">
                                    {{ fmt(carbs, 1) }}
                                </p>
                                <p class="mt-1.5 text-[11px] font-bold uppercase tracking-wide text-amber-400">
                                    Gluc. g
                                </p>
                            </div>
                            <div>
                                <p class="font-mono text-4xl font-black leading-none" style="color: oklch(64% 0.18 25)">
                                    {{ fmt(fat, 1) }}
                                </p>
                                <p
                                    class="mt-1.5 text-[11px] font-bold uppercase tracking-wide"
                                    style="color: oklch(72% 0.16 25)"
                                >
                                    Lip. g
                                </p>
                            </div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    ...
                    <div style="border-radius: 20px; background: oklch(95% 0.03 240)" class="p-4">
                        <p class="mb-0.5 font-mono text-2xl font-black leading-none" style="color: oklch(60% 0.16 240)">
                            {{ fmt(protein, 1) }}g
                        </p>
                        <p class="mb-3 text-[11px] font-bold" style="color: oklch(55% 0.12 240)">Protéines</p>
                        <div class="h-1.5 overflow-hidden rounded-full" style="background: oklch(88% 0.06 240)">
                            <div class="h-full rounded-full" :style="{ width: pPct + '%', background: PC }" />
                        </div>
                        <p class="mt-1.5 text-[11px] font-semibold text-neutral-400">{{ pPct }}%</p>
                    </div>

                    <!-- Glucides -->
                    <div style="border-radius: 20px; background: oklch(95% 0.05 85)" class="p-4">
                        <p class="mb-0.5 font-mono text-2xl font-black leading-none text-amber-600">
                            {{ fmt(carbs, 1) }}g
                        </p>
                        <p class="mb-3 text-[11px] font-bold text-amber-500">Glucides</p>
                        <div class="h-1.5 overflow-hidden rounded-full bg-amber-100">
                            <div class="h-full rounded-full" :style="{ width: cPct + '%', background: CC }" />
                        </div>
                        <p class="mt-1.5 text-[11px] font-semibold text-neutral-400">{{ cPct }}%</p>
                    </div>

                    <!-- Lipides -->
                    <div style="border-radius: 20px; background: oklch(95% 0.04 25)" class="p-4">
                        <p class="mb-0.5 font-mono text-2xl font-black leading-none" style="color: oklch(64% 0.18 25)">
                            {{ fmt(fat, 1) }}g
                        </p>
                        <p class="mb-3 text-[11px] font-bold" style="color: oklch(58% 0.14 25)">Lipides</p>
                        <div class="h-1.5 overflow-hidden rounded-full" style="background: oklch(88% 0.06 25)">
                            <div class="h-full rounded-full" :style="{ width: fPct + '%', background: FC }" />
                        </div>
                        <p class="mt-1.5 text-[11px] font-semibold text-neutral-400">{{ fPct }}%</p>
                    </div>
                </div>
                    ...
                    <div class="divide-y divide-neutral-100">
                        ...
                        >
                            <span
                                ...
                                >{{ row.label }}</span
                            >
                            <span class="shrink-0 font-mono text-sm font-bold text-neutral-900">
                                {{ fmt(row.val, row.d) }}&thinsp;<span
                                    class="font-sans text-xs font-semibold text-neutral-400"
                                    >{{ row.unit }}</span
                                >
                            </span>
                        </div>
                ...
                >
                    <div class="mb-4 flex items-center justify-between">
                        ...
                        <span class="rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-bold text-neutral-500">{{
                            servings.length
                        }}</span>
                    </div>
                    <div class="space-y-1">
                        <button
                            ...
                            class="w-full px-3 py-3 text-left transition-all"
                            style="border-radius: 12px"
                            :class="
                                selectedServingId === item.serving_id ? 'ring-1 ring-green-300' : 'hover:bg-neutral-50'
                            "
                        ...
                        >
                            <div class="flex items-center justify-between gap-4">
                                <div class="min-w-0">
                                    ...
                                    <p v-if="item.metric_serving_amount" class="mt-0.5 text-xs text-neutral-400">
                                        {{ fmt(item.metric_serving_amount) }} {{ item.metric_serving_unit }}
                                    </p>
                                </div>
                                <div class="flex shrink-0 items-center gap-4 text-right">
                                    <div>
                                        <p class="font-mono text-sm font-black text-neutral-900">
                                            {{ fmt(item.calories, 0) }}
                                        </p>
                                        <p class="text-[10px] font-bold text-neutral-400">kcal</p>
                                    </div>
                                    <div>
                                        <p class="font-mono text-sm font-bold" :style="{ color: PC }">
                                            {{ fmt(item.protein, 1) }}
                                        </p>
                                        <p class="text-[10px] font-bold text-neutral-400">prot</p>
                                    </div>
                                    <div>
                                        <p class="font-mono text-sm font-bold text-amber-500">
                                            {{ fmt(item.carbohydrate, 1) }}
                                        </p>
                                        <p class="text-[10px] font-bold text-neutral-400">gluc</p>
                                    </div>
                                    <div>
                                        <p class="font-mono text-sm font-bold" :style="{ color: FC }">
                                            {{ fmt(item.fat, 1) }}
                                        </p>
                ...
                <div
                    ...
                    "
                >
                    <p class="mb-4 text-xs font-bold uppercase tracking-widest text-neutral-400">Ajouter au journal</p>

                    <!-- Portion + Quantité -->
                    <div class="mb-4 grid grid-cols-2 gap-3">
                        <div>
                            <p class="mb-1.5 text-xs font-semibold text-neutral-500">Portion</p>
                            <select
                                v-model="selectedServingId"
                                class="focus:ring-green-500/20 h-10 w-full rounded-xl border border-neutral-200 bg-neutral-50 px-3 text-sm text-neutral-700 focus:border-green-500 focus:outline-none focus:ring-2"
                            >
                        ...
                        <div>
                            <p class="mb-1.5 text-xs font-semibold text-neutral-500">Quantité</p>
                            <input
                    ...
                    <MealSelector v-model="selectedMeal" label="Repas" class="mb-4" />

                    <!-- Bouton -->
                    <button
                        @click="addToJournal"
                        :disabled="addingFood === food.food_id"
                        class="flex h-12 w-full items-center justify-center gap-2 bg-green-500 text-sm font-bold text-white shadow-sm transition-all hover:-translate-y-px hover:bg-green-600 active:translate-y-0 disabled:opacity-50"
                        style="border-radius: 9999px"
                    >
                        <Loader2 v-if="addingFood === food.food_id" :size="16" class="animate-spin" />
                    ...
                    <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        class="mt-3 inline-flex h-9 w-full items-center justify-center gap-1.5 border border-neutral-200 text-sm font-semibold text-neutral-500 transition-colors hover:border-green-300 hover:text-green-600"
                        style="border-radius: 9999px"
                    >
                ...
                <div
                    ...
                    "
                >
                    <p class="mb-4 text-xs font-bold uppercase tracking-widest text-neutral-400">
                        Allergènes & préférences
                    </p>
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div v-if="allergens.length">
                            <p class="mb-3 text-[10px] font-bold uppercase tracking-widest text-neutral-300">
                                Allergènes
                            </p>
                            <div class="space-y-2">
                                <div
