...
import { TrendingDown, Flame, Scale, Target } from 'lucide-vue-next'
...
const props = defineProps({
    ...
    stats: {
        type: Object,
        default: () => ({ avgCalories: 0, currentWeight: null, totalLoss: null, goalCalories: 2000 }),
    },
})
...
const chartData = computed(() => {
    ...
    return DAYS.map((d) => ({ day: d, calories: 0, weight: null }))
})
        ...
        <div class="flex flex-col gap-5 px-6 py-6 md:px-7">
            ...
            <div class="rounded-xl bg-white p-5 shadow-md">
                ...
                <div class="relative">
                    ...
                    <div class="mt-2 flex justify-between px-5">
                        <span v-for="p in points" :key="p.day" class="text-[10px] font-medium text-neutral-400">
                            {{ p.day }}
                        </span>
                    </div>
                </div>
            ...
            <div class="overflow-hidden rounded-lg bg-white shadow-sm">
                <div class="border-b border-neutral-100 px-4 py-3">
                    <p class="text-[11px] font-semibold uppercase tracking-widest text-neutral-400">Détail semaine</p>
                </div>

                <div v-if="weeklyData.length" class="divide-y divide-neutral-50">
                    <div v-for="day in weeklyData" :key="day.date" class="flex items-center gap-4 px-4 py-3">
                        <span class="w-10 shrink-0 text-xs font-semibold text-neutral-500">{{ day.day }}</span>
                        <div class="h-1.5 flex-1 overflow-hidden rounded-full bg-neutral-100">
                            <div
                                class="h-full rounded-full bg-green-500"
                                :style="{
                                    width: `${Math.min((day.calories / (stats.goalCalories || 2000)) * 100, 100)}%`,
                                }"
                            />
                        </div>
                        <span class="w-16 shrink-0 text-right font-mono text-xs text-neutral-600">
                            {{ day.calories }} kcal
                        </span>
                    </div>
                </div>
